...
DROP DATABASE IF EXISTS  madang; /* 만약 마당 테이블이 있으면 지워라 */
DROP USER IF EXISTS  madang@localhost; /* 만약 마당 유저가있으면 지워라 */
...
CREATE TABLE Orders (
  ...
  FOREIGN KEY (bookid) REFERENCES Book(bookid) /*레퍼런스 쳐다본다*/
);
